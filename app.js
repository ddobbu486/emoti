...
/* ===================== 미니 이모티콘 ===================== */
// 미니 이모티콘(멈춰있는 버전) 정식 규격: 180x180px PNG, 총 42개
const MINI_ITEMS = [
  "기쁜 얼굴", "슬픈 얼굴", "놀란 얼굴", "화난 얼굴", "부끄러운 얼굴",
  "우는 얼굴", "졸린 얼굴", "무표정한 얼굴",
  "하트 날리는 손", "엄지척 손", "박수치는 손", "브이 손가락",
  "도리도리 손", "거수경례 손", "기도하는 손", "머리 긁적이는 손",
  "물음표", "느낌표", "땀방울", "반짝임 효과",
  "음표", "체크 표시", "엑스 표시", "졸린 Z Z Z",
  "하트", "별", "폭죽", "한숨 구름",
  "번개", "빈 말풍선", "커피컵", "케이크",
  "선물상자", "시계", "전화기", "우산",
  "눈물방울", "불꽃", "깃발", "리본",
...
const miniResults = new Array(MINI_ITEMS.length).fill(null);
...
function getMiniConceptText() {
  ...
  if (base) return base;
  if (extra) return `[캐릭터 컨셉]\n${extra}`;
  return `[캐릭터 컨셉]\n특별한 설정 없음, 귀엽고 단순한 캐릭터`;
}
...
async function generateMiniItem(label) {
  ...
  const refB64 = charImageBase64 || autoReferenceBase64;
  const refMime = charImageMime || autoReferenceMime;

  const promptText = `카카오 "미니 이모티콘" 스타일의 아주 작고 단순한 아이콘 하나를 그려줘.
미니 이모티콘은 카톡 말풍선 안에서 텍스트와 함께 작게 쓰이는 거라서, 디테일을 최소화하고 한눈에 알아볼 수 있게 그려야 해.

${refB64 ? "함께 첨부한 이미지/캐릭터와 같은 느낌의 색감과 화풍으로," : ""} 이번에 그릴 부품: "${label}"
...
- 아주 단순하고 작게 봐도 알아볼 수 있을 정도로 디테일을 최소화
- "${label}"이 캐릭터의 얼굴/손 일부분이어도 되고, 독립된 사물/효과(하트, 별, 땀방울 등)여도 돼. 전신을 다 그릴 필요 없음
- 다른 부품과 픽셀 단위로 정확히 맞물릴 필요는 없음 (사용자가 직접 손으로 다시 그릴 예정이라 비율과 아이디어만 참고용으로 보여주면 됨)
- 텍스트는 넣지 마`;
...
async function renderMiniCellResult(i, result) {
  ...
  cell.classList.remove("pending", "error");
  const url = `data:${result.mime};base64,${result.base64}`;
  imgBox.innerHTML = `<img src="${url}" alt="${MINI_ITEMS[i]}">`;
  if (!cell.querySelector(".cell-dl")) {
    const dl = document.createElement("div");
    dl.className = "cell-dl";
    dl.textContent = "PNG 저장";
    dl.addEventListener("click", () => {
      const a = document.createElement("a");
      a.href = url;
      a.download = `mini_${String(i + 1).padStart(2, "0")}_${MINI_ITEMS[i].replace(/\s+/g, "_")}.png`;
      a.click();
    });
    cell.appendChild(dl);
  }
...
async function generateMiniRange(startIndex, endIndexExclusive) {
  ...
  async function worker() {
    while (cursor < endIndexExclusive) {
      ...
      const cell = document.getElementById(`mini-cell-${i}`);
      try {
        const r = await generateMiniItem(MINI_ITEMS[i]);
        miniResults[i] = r;
        ...
        cell.querySelector(".cell-img").textContent = "실패";
        cell.title = e.message;
        console.error(`[mini ${i}] ${MINI_ITEMS[i]}`, e);
      }
      done++;
  ...
  miniGenStatus.textContent = `완료! ${successCount} / ${MINI_ITEMS.length}`;
...
generateMiniRestBtn.addEventListener("click", async () => {
  ...
  await generateMiniRange(MINI_PREVIEW_COUNT, MINI_ITEMS.length);
  generateMiniRestBtn.classList.add("hidden");
});

downloadMiniZipBtn.addEventListener("click", async () => {
  const zip = new JSZip();
  zip.file("concept.txt", getMiniConceptText());
  miniResults.forEach((r, i) => {
    if (r) {
      const num = String(i + 1).padStart(2, "0");
      const name = MINI_ITEMS[i].replace(/\s+/g, "_");
      const ext = (r.mime && r.mime.includes("png")) ? "png" : "jpg";
      zip.file(`${num}_${name}.${ext}`, r.base64, { base64: true });
    }
  });
