...
let miniResults = new Array(MINI_TYPES[selectedMiniType].items.length).fill(null);
...
function renderMiniTypeGrid() {
  ...
  Object.entries(MINI_TYPES).forEach(([key, t]) => {
    ...
    btn.addEventListener("click", () => {
      ...
      miniResults = new Array(MINI_TYPES[key].items.length).fill(null);
      renderMiniTypeGrid();
      miniGrid.innerHTML = "";
...
async function generateMiniItem(label) {
  ...
  if (selectedMiniType === "object") {
    ...
    typeInstruction = `이건 "한글자·단어 텍스트형" 미니이모티콘이야. 그림이 아니라 한글 글자 "${label}" 자체를 그래픽 타이포그래피로 디자인해줘. 글자가 화면의 메인 비주얼이고, 필요하면 아주 작은 장식(테두리, 그림자, 색 배경)만 더해. 글자는 크고 명확하게 한눈에 읽혀야 해.`;
  } else if (selectedMiniType === "character") {
    typeInstruction = label.includes("얼굴")
      ? `이건 캐릭터의 얼굴만 아주 작게 표현하는 표정 아이콘이야. 몸 전체를 그릴 필요 없고, 표정 자체가 한눈에 읽혀야 해.`
...
- 세트 내 다른 항목들과 일관된 화풍/색감을 유지`;

  const parts = [{ text: promptText }];
  if (useReference && refB64) parts.push({ inline_data: { mime_type: refMime, data: refB64 } });
...
function buildMiniGrid() {
  const items = MINI_TYPES[selectedMiniType].items;
  miniGrid.innerHTML = "";
...
async function renderMiniCellResult(i, result) {
  ...
  cell.classList.remove("pending", "error");
  const url = `data:${result.mime};base64,${result.base64}`;
  imgBox.innerHTML = `<img src="${url}" alt="${items[i]}">`;
  if (!cell.querySelector(".cell-dl")) {
    const dl = document.createElement("div");
    dl.className = "cell-dl";
    dl.textContent = "PNG 저장";
    dl.addEventListener("click", () => {
      const a = document.createElement("a");
      a.href = url;
      a.download = `mini_${String(i + 1).padStart(2, "0")}_${items[i].replace(/\s+/g, "_")}.png`;
      a.click();
    });
    cell.appendChild(dl);
  }
...
async function generateMiniRange(startIndex, endIndexExclusive) {
  ...
  async function worker() {
    while (cursor < endIndexExclusive) {
      ...
      const cell = document.getElementById(`mini-cell-${i}`);
      try {
        const r = await generateMiniItem(items[i]);
        miniResults[i] = r;
        ...
        cell.querySelector(".cell-img").textContent = "실패";
        cell.title = e.message;
        console.error(`[mini ${i}] ${items[i]}`, e);
      }
      done++;
  ...
  miniGenStatus.textContent = `완료! ${successCount} / ${items.length}`;
  generateMiniBtn.disabled = false;
...
generateMiniRestBtn.addEventListener("click", async () => {
  ...
  await generateMiniRange(MINI_PREVIEW_COUNT, MINI_TYPES[selectedMiniType].items.length);
  generateMiniRestBtn.classList.add("hidden");
});
...
downloadMiniZipBtn.addEventListener("click", async () => {
  ...
  miniResults.forEach((r, i) => {
    if (r) {
      const num = String(i + 1).padStart(2, "0");
      const name = items[i].replace(/\s+/g, "_");
      const ext = (r.mime && r.mime.includes("png")) ? "png" : "jpg";
      zip.file(`${num}_${name}.${ext}`, r.base64, { base64: true });
    }
  });
