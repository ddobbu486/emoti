...
/* ---- 랜덤 컨셉 추천 (아이디어 막힐 때) ---- */
const RANDOM_MINI_CONCEPTS = [
  { type: "object", theme: "식집사(반려식물) 소품 모음 — 화분, 분무기, 다육이, 새싹" },
  { type: "object", theme: "캠핑/차박 용품 모음 — 텐트, 랜턴, 코펠, 캠프파이어" },
  { type: "object", theme: "디저트 카페 메뉴 모음 — 마카롱, 라떼, 와플, 크로플" },
  { type: "object", theme: "여름 바다휴가 소품 모음 — 튜브, 조개, 파라솔, 아이스크림" },
  { type: "object", theme: "자취생 생활밀착 소품 — 택배상자, 분리수거, 라면, 즉석밥" },
  { type: "object", theme: "겨울 크리스마스 소품 모음 — 트리, 양말, 눈사람, 핫초코" },
  { type: "text", theme: "헬스/오운완 운동 용어 — 오운완, PT, 벌크업, 유산소" },
  { type: "text", theme: "직장인 야근·칼퇴 용어 — 칼퇴, 야근, 점심, 보고서" },
  { type: "text", theme: "술자리·회식 용어 — 짠, 원샷, 해장, 2차" },
  { type: "text", theme: "수험생·공시생 용어 — 합격, 멘붕, 벼락치기, 화이팅" },
  { type: "character", theme: "고양이 집사 일상 표정 시리즈" },
  { type: "character", theme: "곰돌이 직장인 리액션 시리즈" },
  { type: "shape", theme: "파스텔톤 도형 채팅 꾸미기" },
  { type: "shape", theme: "네온 컬러 숫자/화살표 세트" }
];

function pickRandomMiniConcept() {
  const pick = RANDOM_MINI_CONCEPTS[Math.floor(Math.random() * RANDOM_MINI_CONCEPTS.length)];
  selectedMiniType = pick.type;
  miniConceptInput.value = pick.theme;
  resetMiniState();
  renderMiniTypeGrid();
}
