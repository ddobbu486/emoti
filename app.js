...
async function generateMiniItem(label) {
  ...
  if (selectedMiniType === "object") {
    ...
    typeInstruction = `이건 "도형·숫자 채팅꾸미기형" 미니이모티콘이야. "${label}"을 심플한 도형/숫자/기호로 그려줘. 배경 처리 방식(① 배경 없음/흰색, ② 모든 칸에 똑같은 모양·색의 단색 배경, ③ 모든 칸에 똑같은 패턴 배경 — 이 중 딱 하나)과 색상 팔레트(2~3가지 포인트 컬러로 고정)를 한 번 정하면, 세트의 모든 항목에 예외 없이 똑같이 적용해야 해. 어떤 항목엔 색깔 배경을 넣고 다른 항목엔 안 넣거나, 항목마다 메인 컬러가 바뀌는 건 절대 안 돼.`;
  ...
  const styleRefInstruction = hasStyleRef
    ? `\n\n중요(반드시 지킬 것): 함께 첨부한 이미지는 이 세트의 첫 번째로 완성된 항목이야. 이번에 그릴 항목도 그 이미지와 정확히 똑같은 색상 팔레트, 똑같은 배경 처리(배경이 있다면 같은 모양·같은 색, 없다면 똑같이 배경 없음), 똑같은 선 굵기와 톤(텍스트형이면 폰트 디자인까지)을 써야 해. 첨부 이미지에 없던 새로운 색이나 새로운 배경 요소(예: 첨부 이미지엔 없는 색깔 동그라미 배경 등)를 임의로 추가하지 마. 그려야 할 대상(주제)만 바뀌고, 나머지 스타일 요소는 첨부 이미지를 그대로 복제하듯 따라가야 해.`
    : "";
...
- 매우 중요: 색상 팔레트와 배경 처리 방식(배경이 있는지 없는지, 있다면 어떤 모양·색인지)을 이 세트의 모든 항목에서 동일하게 고정해야 해. 항목마다 메인 컬러가 바뀌거나, 어떤 항목에만 색깔 배경/도형을 추가하는 식으로 일관성이 깨지면 안 돼`;
...
[실제 카카오 미니이모티콘 스타일 — 가장 중요, 절대 어기지 말 것]
실제 카카오톡 "미니 이모티콘"(예: 미니 문방구, 춘식이의 하루 같은 것들)은 카톡 말풍선 안에 텍스트와 나란히 들어가는 아주 작은 아이콘이야. 방금 같은 입체적이고 광택 도는 3D 클레이 렌더링 스타일은 실제 미니이모티콘과 전혀 달라. 반드시 아래 스타일을 따라줘:
- 완전히 납작한(flat) 디자인. 그라데이션, 입체 음영, 광택 하이라이트, 부드러운 3D 베벨/클레이 질감 절대 금지
- 굵고 깔끔한 단색 외곽선 하나 + 그 안을 평면 색으로 채우는 정도. 그림자도 넣지 않거나, 넣더라도 한쪽에 아주 옅은 단색 그림자 한 줄 정도만
- 디테일을 극도로 절제: 작은 화면에서도 1초 안에 뭔지 알아볼 수 있을 정도로 단순한 실루엣
- 색은 2~4가지로 제한, 채도는 과하지 않게
- 절대 사실적이거나 정교한 일러스트, 컨셉아트, 3D 렌더링 느낌으로 가지 마. 단순하고 친근한 평면 스티커여야 해

[진행 방식 — 반드시 이 순서로]
1. 먼저 1번 항목 "${items[0] || ""}" 하나만 위 스타일대로 그려서 보여줘. 이게 이후 ${items.length - 1}장의 스타일 기준이 될 거야. (내가 보기에 너무 입체적이거나 화려하면 "더 납작하고 단순하게 다시 그려줘"라고 요청할 수 있어)
2. 내가 1번 결과를 확인하고 계속 진행하라고 하면, 2번 항목부터 한 번에 5~6장씩 묶어서 차례로 그려나가줘
3. 매 배치마다 직전에 그린 이미지들의 선 굵기·색감·평면 스타일을 계속 참고해서, 세트 전체가 끝까지 같은 시리즈처럼 보이게 유지해줘
4. 각 이미지를 줄 때 리스트에 적힌 번호와 항목명을 정확히 그대로 "N. 항목명" 형식으로 표시해줘. 번호를 건너뛰거나 중복하지 말고, 리스트 순서를 그대로 따라가줘
...
[그릴 항목 리스트 — 총 ${items.length}개, 항목당 정확히 1장씩, 번호와 순서를 그대로 지킬 것]
${numbered}

[공통 규칙]
- 정사각형 1:1 비율, 180×180px 기준, 배경은 완전 투명 PNG (불가능하면 순수 흰색 배경)
- 한 이미지 안에는 그 항목 단 하나만 그려. 여러 사물을 한 화면에 나열하거나 콜라주/무드보드처럼 모아 그리지 마
- 세트 전체(${items.length}개)가 같은 화풍·색감·선 굵기로 통일되어 같은 시리즈처럼 보여야 해. 항목마다 메인 컬러가 바뀌거나 일부에만 배경/장식이 추가되는 식으로 일관성이 깨지면 안 돼
...
${hasCharImg ? "- 이 프롬프트와 함께 캐릭터 레퍼런스 이미지를 첨부할 거야. 그 이미지 속 캐릭터의 얼굴형·색·특징을 기준으로 통일해줘. 단, 첨부 이미지가 입체적으로 그려져 있더라도 미니이모티콘으로 그릴 때는 위 [실제 카카오 미니이모티콘 스타일] 기준에 맞게 납작하게 단순화해서 그려줘 (이 텍스트만으로는 이미지가 전달되지 않으니, 직접 그 캐릭터 이미지 파일을 함께 첨부해줘)." : ""}
