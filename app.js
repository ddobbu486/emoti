...
- 카카오 이모티콘처럼 굵은 라인 + 완성된 채색 일러스트, 색감은 위 컨셉의 "색감/분위기"를 따를 것
...
async function runPreview() {
  ...
  try {
    ...
    const r0 = await generateOneImage(SITUATIONS[0], refB64, refMime);
    results[0] = r0;
    autoReferenceBase64 = r0.base64;
    autoReferenceMime = r0.mime;

    const url = `data:${r0.mime};base64,${r0.base64}`;
    previewImgBox.innerHTML = `<img src="${url}" alt="미리보기">`;
...
function buildGridWithPreview() {
  ...
  if (results[0]) renderCellResult(0, results[0]);
}
...
async function generateRest() {
  ...
  buildGridWithPreview();
  downloadZipBtn.disabled = true;

  const total = SITUATIONS.length;
  let done = results[0] ? 1 : 0;
  genStatus.textContent = `생성 중... ${done} / ${total}`;

  const referenceB64 = charImageBase64 || autoReferenceBase64;
  const referenceMime = charImageMime || autoReferenceMime;
